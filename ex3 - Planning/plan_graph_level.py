...
class PlanGraphLevel(object):
    ...
    def update_action_layer(self, previous_proposition_layer):
        ...
        "*** YOUR CODE HERE ***"
        relevant_acts = [act for act in all_actions if previous_proposition_layer.all_preconds_in_layer(act)]

        for act in relevant_acts:
            to_add = True

            for prop1 in act.get_pre():
                for prop2 in act.get_pre():
                    if (prop1.get_name() != prop2.get_name()) and previous_proposition_layer.is_mutex(prop1, prop2):
                        to_add = False
                        break
    ...
    def update_mutex_actions(self, previous_layer_mutex_proposition):
        ...
        for act1 in current_layer_actions:
            for  act2 in current_layer_actions:
                if (act1.get_name() != act2.get_name()) and mutex_actions(act1, act2 , previous_layer_mutex_proposition):
    ...
    def update_proposition_layer(self):
        ...
        for act in current_layer_actions:
            for prop in act.get_add():
                if prop.get_name() not in propositions:
                    propositions[prop.get_name()] = Proposition(prop.get_name())

                #if act not in propositions[prop.get_name()].get_producers():
                propositions[prop.get_name()].get_producers().append(act)

        # Add the propositions to the current layer
        for prop in propositions:
            self.proposition_layer.add_proposition(propositions[prop])
    ...
    def update_mutex_proposition(self):
        ...
        for prop1 in current_layer_propositions:
            for prop2 in current_layer_propositions:
                if (prop1.get_name() != prop2.get_name()) and mutex_propositions(prop1 , prop2, current_layer_mutex_actions):
...
def have_competing_needs(a1, a2, mutex_props):
    ...
    for pre1 in a1.get_pre():
        for pre2 in a2.get_pre():
            all_pairs.append(Pair(pre1, pre2))

    return len([pair for pair in all_pairs if pair in mutex_props]) > 0
...
def mutex_propositions(prop1, prop2, mutex_actions_list):
    ...
    """
    "*** YOUR CODE HERE ***"
    achv_prop1 = prop1.get_producers()
    achv_prop2 = prop2.get_producers()

    prop_set = set()

    for act1 in achv_prop1:
        for act2 in achv_prop2:
            prop_set.add(Pair(act1, act2))

    return prop_set.issubset(mutex_actions_list)
    #
    # for act1 in achv_prop1:
    #     for act2 in achv_prop2:
    #         if Pair(act1, act2) in mutex_actions_list:
    #             return False
    #
    # return True